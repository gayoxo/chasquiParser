...
public class Atributos_metadatos_Categoria_Status_ExtendControlledAttribute
	...
	protected void process_Vocabulary() {
		try {
			ResultSet rs=MySQLConnectionChasqui.RunQuerrySELECT("SELECT distinct contenido FROM chasqui2.metadatos WHERE ruta='/manifest/metadata/lom/lifecycle/status/value/langstring' ORDER BY contenido;");
			if (rs!=null) 
			{
				while (rs.next()) {
	...
	private void process_AtributeInstances() {
		try {
			ResultSet rs=MySQLConnectionChasqui.RunQuerrySELECT("SELECT * FROM chasqui2.metadatos WHERE ruta='/manifest/metadata/lom/lifecycle/status/value/langstring' ORDER BY contenido;");
			if (rs!=null) 
			{
				while (rs.next()) {
